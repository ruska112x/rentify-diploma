...
class BookingService(
    ...
    @Transactional
    fun create(addBookingRequest: AddBookingRequest, userId: String) {
        ...
        val rentalListing = rentalListingOptional.orElseThrow {
            ResponseStatusException(
                HttpStatus.NOT_FOUND, "RentalListing with id `${addBookingRequest.rentalListingId}` not found"
            )
        }
    ...
    fun getBookingById(bookingId: String): GetBookingResponse {
        ...
        val booking = bookingOptional.orElseThrow {
            ResponseStatusException(
                HttpStatus.NOT_FOUND, "Booking with id `$bookingId` not found"
            )
        }
        val getBookingResponse = GetBookingResponse(
            booking.id.toString(),
    ...
    fun getBookingsByUserId(userId: String): List<GetBookingResponse> {
        ...
        val getBookingResponseList = bookingList.map {
            GetBookingResponse(
                it.id.toString(),
                it.startDateTime,
                it.endDateTime,
    ...
    fun updateBookingById(bookingId: String, updateBookingRequest: UpdateBookingRequest) {
        val bookingOptional = bookingRepository.findById(UUID.fromString(bookingId))
        val booking = bookingOptional.orElseThrow {
            ResponseStatusException(
                HttpStatus.NOT_FOUND, "Booking with id `$bookingId` not found"
